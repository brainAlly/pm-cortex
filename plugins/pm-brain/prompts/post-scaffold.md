...
- Customer interview → `brain/source/interviews/`, `brain/ingestion/interviews/`, `brain/knowledge/users/insights.md`, relevant `brain/hypotheses/<feature>.md`.
- Meeting / 1:1 → `brain/source/meetings/`, `brain/ingestion/meetings/`, `brain/stakeholders/<slug>.md`, draft `brain/decisions/` when a decision was made.
- Market intel → `brain/source/market/`, `brain/ingestion/market/`, `brain/knowledge/market/competitors/` or `trends.md`, possibly `brain/knowledge/strategy.md § Tensions`.
- Ad-hoc → `brain/ingestion/adhoc/`, routed to the right durable area in-session, never parked.

### 1b. The 6 core commands resolve to their spec files

Our system ships 45 commands total. This self-test checks the 6 core routing commands. For each, confirm the spec exists and the loaded/updated paths in the spec match real scaffold paths:
...
- **Feature → hypothesis.** For every feature file in `brain/knowledge/product/features/` with a status in {scoping, building, shipping, measuring}, expect a corresponding `brain/hypotheses/<slug>.md`. Missing → surface as a next move, not a blocker.
- **Hypothesis ↔ feature.** Every `brain/hypotheses/<slug>.md` should reference its feature file in `## Meta § Feature`. Orphans → flag.
...
- **Stakeholder touchpoints → ingestion.** Every touchpoint-log entry in `brain/stakeholders/<slug>.md` that came from a meeting should link to its `brain/ingestion/meetings/<file>`. Standalone touchpoints (no ingestion artifact) are fine; orphan links (pointing at missing files) are not.
...
- **Where it lives in the brain:** `brain/knowledge/strategy.md § Tensions § Tn`, stakeholder file, hypothesis file
