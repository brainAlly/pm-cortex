...
Create `brain/source/` at the project root. **Copy** pre-existing PM artifacts into it. **Do not move them.** Leave the originals where the PM put them.
...
- **`brain/source/` is the canonical immutable copy.** Once an artifact is in `brain/source/`, treat it as read-only for the rest of the system. Knowledge promotion cites this copy.
- **Originals stay put.** The PM can delete originals later when they're confident — but the skill does not assume that authority.
- **Confirm before bulk operations.** If you're about to copy more than ~10 files or anything larger than ~1 MB total, list what you're copying and confirm in one line: "About to copy N files (X MB) into `brain/source/`. Proceed?"
- **Never overwrite.** If `brain/source/` already exists with content, pause and ask. Do not silently merge.

Everything downstream cites the `brain/source/` copy. This is the provenance chain.
...
Synthesized output lands in `brain/ingestion/` first. Promotion into `brain/knowledge/` follows the standard memory promotion bar (recurring, decision-relevant, strategy-relevant, observed across sources, useful beyond one session) — see `CLAUDE.md § Memory promotion`.

## 2a. Per-artifact ingestion records are scoped to ONE artifact — HARD RULE

Each `brain/ingestion/<kind>/<date>-<slug>.md` is the synthesis of **its matching `brain/source/<kind>/<date>-<slug>.md` and nothing else.**
...
**DON'T (inside `brain/ingestion/strategy/2025-q4-strategy.md`):**
...
**DO (inside `brain/ingestion/strategy/2025-q4-strategy.md`):**
...
**Pre-save self-check before writing any `brain/ingestion/<kind>/<file>.md`:**

1. Does this file contain ANY claim, reference, or comparison that did not appear in its matching `brain/source/` file? If yes — remove it. Route it to the contradiction list.
...
```
brain/source/      →   brain/ingestion/  →   brain/knowledge/    (durable observations, synthesized facts)
(immutable copy)       (working memory)      brain/hypotheses/   (testable beliefs)
                                             brain/decisions/    (committed choices)
                                             brain/stakeholders/ (people state)
```

Promotion is parallel. The same ingested artifact can land evidence in multiple durable areas at once — `brain/knowledge/` is not the only destination. A single customer interview often updates `brain/knowledge/users/insights.md`, strengthens evidence in a `brain/hypotheses/<feature>.md`, and logs a touchpoint in `brain/stakeholders/<slug>.md`. Sometimes it drafts a `brain/decisions/` entry too.

Every claim in any durable area carries a **provenance tag** from the enum in `brain/hypotheses/_SCHEMA.md`. Most claims will tag back through `brain/ingestion/` to a `brain/source/` artifact — that's the canonical chain. But the system enforces vocabulary, not workflow: a claim heard verbal-only from a stakeholder, or inherited from PM intuition, is legitimate as long as it's tagged honestly. The audit promise is "every claim wears its source," not "every claim was synthesized."

Note: `brain/source/` is a **copy** of what the PM gave us. Their originals still live wherever they put them. Promotion cites the `brain/source/` copy by relative path, not the original location.
...
After ingest, you already know much of what the 6-batch interview would ask. Open the interview by telling the PM what you already have:
