...
- `brain/decisions/_SCHEMA.md` — load before writing the decision file
- `brain/hypotheses/<slug>.md` for any hypothesis the decision resolves (the decision MUST link back to it)
- `brain/knowledge/strategy.md` — priorities, non-goals, tensions (does this decision honor strategy? create new tension?)
- Recent `brain/ingestion/` and `brain/source/` referenced as evidence (every Evidence row in the decision file needs a provenance tag pointing at a real artifact)
- `brain/stakeholders/<slug>.md` for any stakeholder named in the decision (Naomi confirmed Q3, Helena flagged budget, etc.)
- `brain/decisions/INDEX.md` to check for prior decisions this one supersedes or depends on
...
- **COUNT-THE-TAGS** before saving — every Evidence row + every "Explicitly NOT doing" row carries a provenance tag from the enum in `brain/hypotheses/_SCHEMA.md`. Orphan rows fail the audit.
