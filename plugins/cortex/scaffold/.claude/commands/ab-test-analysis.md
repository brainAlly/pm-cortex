...
Load if available:
- Active `brain/hypotheses/` for the feature under test — cross-reference the hypothesis being tested and update confidence after the result
- `brain/knowledge/product/metrics.md` — baseline metrics to contextualize the lift
...
Save to `outputs/<date>-ab-test-<slug>.md`.
...
- "Want me to **update the hypothesis in `brain/hypotheses/`** with this test result?"
