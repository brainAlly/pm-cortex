# Product Roadmap Template

Your strategic communication tool for aligning teams and stakeholders.

## Why Roadmaps Matter

"The format matters as much as the content. A poorly communicated roadmap leads to misaligned teams, confused stakeholders, and missed opportunities." - Aakash Gupta

**Choose format based on audience:**
- Executive: High-level themes, business outcomes
- Engineering: Timeline, dependencies
- Sales/Marketing: Features, launch dates
- Design: User value, experience improvements

---

## Roadmap Header

**Product:** [Product name]  
**Time Period:** [Q1 2026 / 2026 / Next 12 months]  
**Owner:** [PM name]  
**Last Updated:** [Date]  
**Next Review:** [Date]

**Strategic Context:**
- North Star Metric: [e.g., Monthly Active Users]
- Current Value: [e.g., 50K MAU]
- Target: [e.g., 100K MAU by Q4]

**Company OKRs this roadmap supports:**
- OKR 1: [Objective + Key Results]
- OKR 2: [Objective + Key Results]

---

## Now / Next / Later Framework

### NOW (This Quarter)

The NOW table mirrors the canonical work table in `brain/knowledge/product/roadmap.md § Now` — the single home for open work. Use the **same columns** so a row copies straight across in either direction, and `/review` and `/sprint` (which own that table) keep it current. Every row needs an `Owner` and a `Due` (a date, or the literal `unscheduled`); `Source` links back to the finding that created it.

**Theme 1: [Strategic Theme]** *(optional grouping — keep the columns below)*
*Why: [Business rationale]* · *OKR: [Which OKR this supports]*

| Item | Owner | Due | Status | Source | Target outcome |
|---|---|---|---|---|---|
| [Feature 1] | [Owner] | [YYYY-MM-DD or unscheduled] | todo | [hypothesis slug / decision file / review log] | [what will be measurably different] |
| [Feature 2] | [Owner] | [YYYY-MM-DD or unscheduled] | in-progress | [source link] | [target outcome] |

`Status` is one of `todo` · `in-progress` · `blocked` · `done`.

**Theme 2: [Strategic Theme]**
[Same table structure]

**NOW Success Criteria:**
- [Metric 1 target]
- [Metric 2 target]
- [Metric 3 target]

---

### NEXT (Next Quarter)

**Theme 1: [Strategic Theme]**
*Why: [Business rationale]*

**Top Priorities:**
1. [Initiative]: [Description and expected impact]
2. [Initiative]: [Description and expected impact]
3. [Initiative]: [Description and expected impact]

**Dependencies:**
- [What needs to happen first]
- [What needs to happen first]

---

### LATER (Future / Backlog)

**Under Consideration:**
- [Initiative]: [Why it's interesting but not prioritized yet]
- [Initiative]: [Why it's interesting but not prioritized yet]

**Explicitly NOT Doing:**
- [Initiative]: [Why we decided against it]
- [Initiative]: [Why we decided against it]

---

## Alternative Format: Timeline Roadmap

### Q1 2026

**Jan-Feb: [Theme]**
- Week 1-2: [Initiative/Milestone]
- Week 3-4: [Initiative/Milestone]
- Week 5-8: [Initiative/Milestone]

**March: [Theme]**
- [Initiative/Milestone]
- [Initiative/Milestone]

### Q2 2026
[Same structure]

### Q3 2026
[High-level themes only]

---

## Alternative Format: Goal-Oriented Roadmap

### Goal 1: [Increase activation rate from 40% to 60%]

**Why:** New users struggle with onboarding (65% drop at step 3)

**How:**
| Initiative | Expected Impact | Timeline | Status |
|------------|----------------|----------|--------|
| Personalized onboarding | +15% activation | Q1 | In progress |
| Onboarding video tutorials | +5% activation | Q2 | Planning |
| Smart defaults | +10% activation | Q2 | Not started |

**Success Metrics:**
- Activation rate: 40% → 60%
- Time to first value: 10 min → 5 min
- D7 retention: +10%

### Goal 2: [Next goal]
[Same structure]

---

## Features vs Outcomes

### ❌ Feature-Focused (Bad)
- Dark mode
- API v2
- Mobile redesign
- Advanced search

### ✅ Outcome-Focused (Good)
- **Reduce churn by 15%** via: Dark mode (accessibility), improved performance
- **Enable enterprise** via: API v2 (integrations), SSO, audit logs
- **Increase mobile engagement** via: Mobile redesign, push notifications
- **Improve discoverability** via: Advanced search, recommendations

---

## Risk Management

### High-Risk Items
| Initiative | Risk | Likelihood | Impact | Mitigation |
|------------|------|------------|--------|------------|
| [Feature] | [What could go wrong] | High/Med/Low | High/Med/Low | [Plan] |

### Dependencies
| Initiative | Depends On | Owner | Status | Risk |
|------------|------------|-------|--------|------|
| [Feature A] | [Feature B] | [Team] | On track | 🟢 |

---

## Confidence Levels

**High Confidence (committed):**
- [Initiative]: We're committed to shipping this quarter
- [Initiative]: Customer contracts depend on this

**Medium Confidence (likely):**
- [Initiative]: Likely but subject to prioritization
- [Initiative]: Dependencies could shift timeline

**Low Confidence (exploring):**
- [Initiative]: Early exploration, may not happen
- [Initiative]: Waiting on validation

---

## Quick Reference by Audience

### For Executives
**Show:** Themes, business outcomes, OKR alignment  
**Hide:** Technical details, specific dates  
**Format:** Now/Next/Later or Goal-Oriented

### For Engineering
**Show:** Features, dependencies, timeline  
**Hide:** Marketing fluff  
**Format:** Timeline with dependencies

### For Sales/Marketing
**Show:** Customer-facing features, launch dates  
**Hide:** Technical debt, infrastructure  
**Format:** Timeline with feature descriptions

### For Customers
**Show:** Value delivery, problem solving  
**Hide:** Internal initiatives, specifics  
**Format:** High-level themes only

---

## Roadmap Best Practices

### Do:
✅ Lead with "why" not just "what"  
✅ Show trade-offs made  
✅ Update regularly (monthly minimum)  
✅ Get stakeholder buy-in  
✅ Make it visual and scannable  
✅ Link to OKRs/strategy

### Don't:
❌ Promise specific dates far out  
❌ Include every small feature  
❌ Make it static (keep it living)  
❌ Use jargon for external audiences  
❌ Ignore dependencies  
❌ Forget to explain the "why"

---

## Maintenance Schedule

**Weekly:** Review progress, update status  
**Monthly:** Update "Now" section, refine "Next"  
**Quarterly:** Major review, update all sections  
**As needed:** Adjust for strategic changes

---

## Tools & Formats

**Best tools by use case:**
- Quick visual: Miro, FigJam
- Integrated with work: Jira, Asana, Monday.com
- Executive presentations: ProductPlan, Roadmunk, PowerPoint
- Collaborative planning: Confluence, Notion
- Lightweight: Google Sheets, Airtable

**Format recommendations:**
- Now/Next/Later: Best for outcome-focused teams
- Timeline: Best for date-driven organizations
- Goal-Oriented: Best for OKR-driven teams
- Swimlane: Best for multiple product lines

---

## Example: Real Roadmap

### Q1 2026 Product Roadmap - [Product Name]

**Strategic Focus:** Enable enterprise adoption

**NOW (Jan-Mar 2026)**

**🔒 Enterprise-Ready**
Why: 60% of pipeline requires these features

- SSO/SAML integration (8 weeks) → Unblocks $2M pipeline
- Advanced permissions (6 weeks) → Required by 70% of enterprise trials
- Audit logs (3 weeks) → Compliance requirement
- Status: On track for March launch

**📊 Data & Insights**
Why: Users can't prove ROI to leadership

- Advanced analytics dashboard (4 weeks) → Requested by 50% of customers
- Custom reports (3 weeks) → Enable customer success conversations
- Status: In progress, launching February

**NEXT (Apr-Jun 2026)**

**🔗 Integrations**
- Salesforce integration → 40% of prospects requested
- Slack notifications → Improve engagement
- API rate limiting → Technical necessity

**LATER (Exploring)**

- Mobile app redesign → High user demand but not enterprise blocker
- AI-powered insights → Interesting but unproven ROI
- Custom branding → Nice-to-have for enterprise

**NOT DOING:**
- Bulk operations → Enterprise doesn't need this
- Social features → Not strategic focus

---

**Last updated:** [Date]  
**Questions?** Contact [PM name] or comment below

---

Use this template to transform your roadmap from a list of features into a strategic communication tool that drives alignment and action.
